          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: () async {
                ...
                // Сохранение API-ключа только в Supabase
                try {
                  await http.post(
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('✅ API-ключ KIE.AI успешно сохранён в Supabase!'),
          ...
          Container(
            ...
            child: const Column(
              ...
              children: [
                ...
                SizedBox(height: 6),
                Text('📸 Фото (Nano Banana 2): ~\$0.04 - \$0.09 (8–18 кредитов) ≈ 3.5–8 сом за генерацию', style: TextStyle(color: Colors.white54, fontSize: 11)),
                SizedBox(height: 2),
                Text('🎬 Видео 10с (Gemini Omni 1080P With Video Input): ~\$0.84 (168 кредитов) ≈ 75 сом за генерацию', style: TextStyle(color: Colors.white54, fontSize: 11)),
                SizedBox(height: 2),
...
class _VipTrendsPageState extends State<VipTrendsPage> with TickerProviderStateMixin {
  ...
  void _startRecording() {
    final int durationMs = _timerDuration * 1000; // 5, 8 или 10 секунд
    const int stepMs = 50;
    int elapsed = 0;
            ...
            child: Stack(
              children: [
                ...
                Positioned(
                  ...
                  child: Column(
                    children: [
                      // Настраиваемый таймер записи (5, 8, 10 секунд)
                      if (_countdownValue == 0 && !_isRecording)
                        Padding(
                          padding: const EdgeInsets.only(bottom: 16.0),
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [5, 8, 10].map((sec) {
...
Future<void> saveTemplatesToStorage() async {
  ...
  try {
    ...
    debugPrint('Templates saved to Supabase.');
  } catch (e) {
